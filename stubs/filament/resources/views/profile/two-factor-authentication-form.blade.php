            ...
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        ...
                        <x-filament-companies::input id="code" type="text" name="code" class="block mt-1 w-1/2" inputmode="numeric" autofocus autocomplete="one-time-code"
        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-filament-companies::confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-filament-companies::button type="button" wire:loading.attr="disabled">
                        {{ __('Enable') }}
            ...
            @else
                @if ($showingRecoveryCodes)
                    <x-filament-companies::confirms-password wire:then="regenerateRecoveryCodes">
                        <x-filament-companies::secondary-button class="mr-3">
                ...
                @elseif ($showingConfirmation)
                    <x-filament-companies::confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-filament-companies::button type="button" class="mr-3" wire:loading.attr="disabled">
                            {{ __('Confirm') }}
                        </x-filament-companies::button>
                    </x-filament-companies::confirms-password>
                @else
                ...
                @if ($showingConfirmation)
                    <x-filament-companies::confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-filament-companies::secondary-button wire:loading.attr="disabled">
                            {{ __('Cancel') }}
                        </x-filament-companies::secondary-button>
                    </x-filament-companies::confirms-password>
                @else
                    <x-filament-companies::confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-filament-companies::danger-button wire:loading.attr="disabled">
                            {{ __('Disable') }}
